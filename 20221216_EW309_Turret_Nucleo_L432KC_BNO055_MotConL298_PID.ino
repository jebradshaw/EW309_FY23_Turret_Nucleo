...
MotConL298 motPitch = MotConL298(A5, A3, A4);
MotConL298 motYaw = MotConL298(A1, A2, A0);
...
void testMotors(void){
  while(1){
    ...
    Serial.printf("%7.2f %7.2f %7.2f %.3f  %.3f\r\n\r\n", orientationData.orientation.x, orientationData.orientation.y, orientationData.orientation.z, motPitch.read(), motYaw.read());
    delay(10);   
  
    if(Serial.available()){
      char c = Serial.read();
      if(c == 'q' || c == 'Q')
        motPitch.mot_control(0.0);
        motYaw.mot_control(0.0);
        return;  
    }  // q for quit
  }// while(1)
...
void setup() {
  ...
  // hardware timers for PWM frequency
  pwmtimer1.pause();
  pwmtimer1.setPrescaleFactor(1); // no divide clock -> Timer2 at 
  pwmtimer1.setOverflow(2000);
  pwmtimer1.refresh();
  pwmtimer1.resume();
//    
//  pwmtimer2.pause();
//  pwmtimer2.setPrescaleFactor(1); // no divide clock -> Timer2 at 
//  pwmtimer2.setOverflow(2000);
//  pwmtimer2.refresh();
//  pwmtimer2.resume();
...
//        pitchMeas = -(PI - yawMeas) - PI;
  ...
  Serial.printf("%7.2f %7.2f yKp=%.2f yKi=%.2f yKd=%.2f pSP=%.3f motp=%.3f  ySP=%.3f moty=%.3f Pinput=%.3f\r\n", 
        yawMeas,
        pitchMeas, 
        yawKp, yawKi, yawKd,
        pitchSetpoint, 
